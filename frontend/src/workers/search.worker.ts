import { 
    env, AutoTokenizer, AutoProcessor, 
    SiglipTextModel, SiglipVisionModel, RawImage 
...
self.addEventListener('message', async (event) => {
    ...
    try {
        if (type === 'init') {
            ...
            const descriptions = items.map((item: any) => item.descriptionEn);
            ...
            for (let i = 0; i < items.length; i++) {
                const textVector = textOutput.data.slice(i * embeddingSize, (i + 1) * embeddingSize);
